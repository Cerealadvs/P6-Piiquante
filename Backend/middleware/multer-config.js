...
const storage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    //Garde le nom du fichier d'origine mais en supprime les espaces pour les remplacer par des _ pour éviter de potentiels problèmes ou erreurs.
    const name = file.originalname.split(" ").join("_");
    //Appel du dictionnaire pour gérer les extensions.
    const extension = MIME_TYPES[file.mimetype];
    //Création du nom de fichier : null pour indiquer qu'il n'y a pas d'erreur, on ajoute le nom du fichier puis un timestamp pour l'heure à laquelle il est créé, puis un point '.', puis l'extension du fichier.
    callback(null, name + Date.now() + "." + extension);
  },
});

// On exporte le middleware multer en appelant le storage en argument, la méthode single pour indiquer qu'il s'agit d'un fichier unique.
